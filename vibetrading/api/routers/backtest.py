...
from vibetrading.persistence.orm_models import BacktestRunORM
...
async def run_backtest(
    ...
    result = await backtest_agent.run_and_persist(session, stock, start_date, end_date, warmup_days=payload.warmup_days)
...
@router.get("/{run_id}")
async def get_backtest(run_id: int, session: AsyncSession = Depends(get_db)) -> dict:
...
@router.get("")
async def list_backtests(symbol: str, session: AsyncSession = Depends(get_db)) -> list[dict]:
    runs = await list_backtest_runs_for_stock(session, symbol.upper())
